...
async def verify_transaction(request: TransactionVerifyRequest, db: AsyncSession = Depends(get_db)):
...
@router.get("/summary")
async def get_verification_summary(db: AsyncSession = Depends(get_db)):
